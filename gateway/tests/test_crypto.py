...
FRAME_CT = "e0f2ae2cf9917ab0c87847f9b6d5bb49ae7f5ce178"
FRAME_TAG = "e5b03bdb72bfbac252ce39f50b67186f"
PLAINTEXT = b'{"n":35,"s":12,"s":1}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([35]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, b"\x23", PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, b"\x23", ciphertext, tag)
